...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, c1, reduction=16):
        ...
        - reduction: Reduction ratio for the channel dimension (default is 16).
        """
        super().__init__()
        self.fc1 = nn.Conv2d(c1, c1 // reduction, kernel_size=1)
        self.fc2 = nn.Conv2d(c1 // reduction, c1, kernel_size=1)
        self.act1 = nn.ReLU()
        self.act2 = nn.Sigmoid()

    def forward(self, x):
        """
        Forward pass that applies squeeze and excitation operations.
        - Squeeze: Applies global average pooling to generate channel descriptors.
        - Excitation: Reweights the channels using learned scaling factors.
        """
        # Squeeze: Global Average Pooling
        scale = x.mean((2, 3), keepdim=True)
        # Excitation: Fully connected layers with non-linear activations
        scale = self.act1(self.fc1(scale))
        scale = self.act2(self.fc2(scale))
        # Scale: Multiply input by the channel-wise attention weights
        return x * scale


class SEConv(nn.Module):
    """Convolution squeeze-and-excitation (CSE)"""

    def __init__(self):
        super().__init__()
        self.conv1 = Conv(3, 16, k=3)
        self.se = SqueezeExcitation(16)
        self.conv2 = Conv(16, 32, k=3)
...
class PWConv(nn.Module):
    """Pointwise convolution with args(ch_in, ch_out, activation)."""

    default_act = nn.SiLU()  # default activation

    def __init__(self, c1, c2, k=1, s=1, p=None, d=1, act=True):
        """Initialize Pointwise Conv layer with 1x1 convolution."""
        super().__init__()
        # stride can't not be 1
    ...
    def forward(self, x):
        """Apply pointwise convolution, batch normalization, and activation to input tensor."""
        # x = self.conv(x)
    ...
    def forward_fuse(self, x):
        """Apply pointwise convolution and activation without batch normalization."""
        return self.act(self.conv(x))


class DConv(nn.Module):
    def __init__(self, c1, k=3, s=1, p=None, d=1, act=True):
        super().__init__()
        self.depthwise = nn.Conv2d(
            c1,
            c1,
            kernel_size=k,
            stride=s,
            padding=autopad(k, p, d),  # Automatically calculate padding
            groups=c1,
            dilation=d,
            bias=False,
        )
...
#     This module replaces traditional convolution by using a depthwise convolution
#     """
...
#     def forward(self, x):
#         """
#         Forward pass applying depthwise convolution, batch normalization, and activation.
#         """
#         x = self.depthwise(x)
#         return self.act(self.bn(x))
