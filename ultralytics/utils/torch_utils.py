...
def dsconv_flops_counter(module, input, output):
    in_channels = module.depthwise.in_channels
    out_channels = module.pointwise.out_channels
    kernel_size = module.depthwise.kernel_size[0]
    input_height, input_width = input[0].shape[2:]

    # Verify if input channels match expected
    if input[0].shape[1] != in_channels:
        raise ValueError(
            f"Channel mismatch in DSConv: expected {in_channels} but got {input[0].shape[1]}"
        )

    # FLOPs for depthwise and pointwise convolutions
    depthwise_flops = input_height * input_width * kernel_size * kernel_size * in_channels
    pointwise_flops = input_height * input_width * in_channels * out_channels
    total_flops = depthwise_flops + pointwise_flops
    module.__flops__ = total_flops  # Store the FLOPs for thop
...
#     if not thop:
#         return 0.0  # if thop is not installed, return 0.0 GFLOPs
#     try:
#         # Define custom operations for DSConv
#         custom_ops = {DSConv: dsconv_flops_counter}
#         model = de_parallel(model)
#         p = next(model.parameters())

#         # Define the input channel configuration based on the model's first layer
#         input_channels = 3  # Default to 3 channels for color images
#         # Get the first layer in the model
#         first_layer = next(model.children())
#         if isinstance(first_layer, DSConv):
#             input_channels = first_layer.depthwise.in_channels  # Set according to DSConv layer
...
#             # Create an input tensor with the correct number of channels
#             im = torch.empty((1, input_channels, stride, stride), device=p.device)  # input image in BCHW format

#             # Calculate FLOPs with custom DSConv operations
#             flops = thop.profile(deepcopy(model), inputs=[im], custom_ops=custom_ops, verbose=False)[0] / 1e9 * 2
#             return flops * imgsz[0] / stride * imgsz[1] / stride  # imgsz GFLOPs

#         except Exception:
#             # Use actual image size for input tensor if exception occurs
#             im = torch.empty((1, input_channels, *imgsz), device=p.device)  # input image in BCHW format
#             return thop.profile(deepcopy(model), inputs=[im], custom_ops=custom_ops, verbose=False)[0] / 1e9 * 2  # imgsz GFLOPs
